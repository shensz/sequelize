...
var QueryCons = require('./query-const');
  ...
  var
    ...
    , buildIncludeMap = function (piece) {
      if ($current.includeMap[piece]) {
        ...
        if (previousPiece) {
          previousPiece = previousPiece+ QueryCons.TABLE_SEPARATOR + piece;
        } else {
          previousPiece = piece;
        }
    ...
    , keyPrefix = function (key) {
      ...
      if (!keyPrefixMemo[key]) {
        ...
        if (!keyPrefixMemo[prefixString]) {
          keyPrefixMemo[prefixString] = prefixString ? prefixString.split(QueryCons.TABLE_SEPARATOR) : [];
        }
    ...
    for (keyI = 0; keyI < keyLength; keyI++) {
      ...
      if ($prevKeyPrefix !== undefined && $prevKeyPrefix !== $keyPrefix) {
        if (checkExisting) {
          ...
          if (length) {
            for (i = 0; i < length; i++) {
              prefix = $parent ? $parent+QueryCons.TABLE_SEPARATOR+$prevKeyPrefix[i] : $prevKeyPrefix[i];
